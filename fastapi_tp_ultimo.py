...
def variaciones(modelo: str, date: str):
    ...
    cur.execute(f"SELECT t1.advertiser_id, COUNT(t1.product_id) FROM tabla_products5 t1 LEFT JOIN tabla_products5 t2 on t1.product_id = t2.product_id AND t2.date = '{date2}' WHERE t1.date = '{date1}' AND t2.product_id IS NULL GROUP BY t1.advertiser_id")
    rtdo = cur.fetchall()
